  ...
  return (
    <div className="min-h-screen bg-gray-900 dark:bg-gray-900">
      ...
      {membersChunks.map((chunk, chunkIndex) => (
        <div
          key={chunkIndex}
          className="flex flex-wrap items-center justify-center my-0"
        >
          {chunk.map((member, index) => (
            <div
              key={index}
              className="w-full mx-2 my-4 rounded-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/4 bg-gradient-to-b from-white via-transparent to-transparent"
            >
              <div className="top-0 left-0 w-full h-full ">
                ...
                <div className="px-6 py-4">
                  <div className="w-full mb-2 text-xl font-bold text-gray-800 dark:text-white">
                    {member.name}
                  </div>
                  <p
